...
- **Prosjektspesifikke minner skal være git-tracked:** Alle prosjektspesifikke fakta, konvensjoner, preferanser og kontekst skal lagres i **git-tracked filer** — `CLAUDE.md`, `TASKS.md`, `README.md`, `historikk.md`, `ARCHIVE.md` eller egne dokumenter i `bakgrunn/`. Bruk **aldri** lokale minnesystemer som `.claude/projects/.../memory/` for prosjektspesifikke ting. Grunn: brukeren jobber på flere maskiner (Windows Git Bash, WSL2, Linux) og trenger at all prosjektkontekst følger med `git clone`. Lokale minner går tapt ved maskinbytte og skaper divergens mellom sesjoner. Kun helt generiske Claude-preferanser (verktøybruk-vaner, generelle arbeidsstiler som ikke er prosjektspesifikke) kan ligge i lokale minner.
