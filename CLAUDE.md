...
## Kontaktinfo og sentrale fakta

**KodeWorks Eiendom AS**

- Org.nr.: **920 478 506** (verifisert mot Brønnøysundregisteret 2026-08-05)
- NB: `920 478 492` er KodeWorks Trondheim AS, et annet selskap – ikke forveksle

**Eirik Larsen** (daglig leder, KodeWorks Eiendom AS)

- E-post: eirik.larsen@kodeworks.no
- Telefon: +47 416 70 269

Bruk disse verdiene direkte i leveranser, søknader og skjemaer — ikke sett `[SETT INN]`-placeholdere og ikke be brukeren fylle inn manuelt. Verifiser mot Brreg / proff.no ved tvil. Fisjon (planlagt) vil gi Fjordgata 30 AS eget org.nr. for driftsselskapet — sjekk aktuell status før viktige leveranser signeres.

**MVA-strategi (kjerneprinsipp):**

- Passiv minilager (kunde med eksklusiv tilgang til fast avgrenset areal) er **MVA-unntatt** etter mval. § 3-11 første ledd
- Aktiv lagringstjeneste («valet storage» — henting, registrering, pakking, levering) er **avgiftspliktig** og gir full fradragsrett for input-MVA i byggefasen
- Anbefalt driftsmodell er hybrid (Alternativ D): kontor og B2B-lager på frivillig MVA-registrering, full-service-enheter som avgiftspliktig aktiv tjeneste. Estimert MVA-dekning i byggefasen: 90–100 % → ca. 5–6 MNOK refusjon
- Sentral rettskilde: Skattedirektoratets prinsipputtalelse 18.11.2014
- Detaljer og rettskilder: `forretningsplan/fg30_vurderinger_mva.md` og `forretningsplan/kilde_mva_regelverk.md`
- Neste kritiske steg: BFU (bindende forhåndsuttalelse) fra Skatteetaten — se aktuell status i `TASKS.md`. MVA-registrering skal skje før ferdigstillelse av bygget.
...
- **Ikke referer til task-IDer i prosa:** Skriv aldri «T140-avtalen», «per T145», «under T143» eller lignende — verken i chat, i beskrivelser i `TASKS.md`, i `historikk.md`, i `leveranser/` eller i dokumenter i `bakgrunn/`. Task-IDer er interne anker og hører kun hjemme (a) som `### T<n>`-overskrift på selve tasken, (b) i **Berørte filer**- og **Kobling**-blokkene på tasken, (c) i commit-meldinger, og (d) i kodekommentarer der en spesifikk beslutning må siteres. Alle andre steder: navngi konseptet. «Ains arbeidsavtale», «kjeller-arbeidsavgrensningen», «MVA-strategien» leses uten oppslag; «T140», «T143» krever at leseren husker hvilken oppgave det var. Symptom å se etter i egne utkast: naken `T<n>` i prosa, «T<n>-avtalen», eller en setning hvis mening krever at leseren kjenner tittelen på en oppgave. Særlig i chat-svar: brukeren har over hundre task-IDer og bærer ikke innholdet i hver enkelt i hukommelsen.
...
- **Installasjon av systempakker eller verktøy krever eksplisitt tillatelse:** Kommandoer som installerer eller endrer systemnivå (`tlmgr install`, `apt install`, `pip install --system`, `pip install --user`, `npm install -g`, `choco install`, `winget install`, `uv tool install`, `cargo install`, `gem install` osv.) krever alltid eksplisitt godkjenning i den aktuelle samtalen — også når et verktøy mangler for å fullføre den etterspurte oppgaven. Stopp og spør. Å installere «bare det som mangler» er nettopp autopilot-fellen i «Spør før suboptimal fremgangsmåte»-regelen.
- **`rm` og andre destruktive operasjoner krever eksplisitt tillatelse for filer jeg ikke selv har opprettet i den aktuelle samtalen.** Før hver `rm`, `mv --force`, `> fil` (overskriv) eller lignende: sjekk om filen er noe jeg selv har generert i denne sesjonen. Hvis ikke — stopp og spør. Signaler på at filen er brukerens (og skal ikke røres): filstørrelse eller endringsdato som avviker fra min egen output, filnavn som ikke matcher min pipeline, filer som «virker rare å ha» i mappen. Å rydde bort «noe som ikke skulle vært der» er autopilot-fellen: brukeren kan ha lagt inn filen med hensikt. Å slette er verre enn å la ligge — spør heller.
- **Norske personnummer og D-nummer — sjekk formatet før du flagger som ugyldig.** 11-sifret norsk ID kan være enten fødselsnummer eller D-nummer. Fødselsnummer starter med DDMMYY (dag 01–31). D-nummer (tildeles utenlandske arbeidstakere) har samme struktur, men **dagsifrene er +40** (dag 41–71) — så en person født 06.06.1973 har D-nummer som starter med `46 06 73 ...` (06+40=46). Sjekk begge norske formater før du antar at et 11-sifret nummer er utenlandsk (estisk isikukood eller lignende). Særlig relevant her fordi flere ansatte (Ain, Marko) er estiske men jobber i Norge og trenger norsk ID for A-melding.
- **La eksterne parter (bank, tilskuddsgivere) gjøre sine egne analyser — ikke bestill tredjepartsvurderinger for å «overbevise».** For bankpakka, tilskuddssøknader og lignende: presenter datagrunnlaget direkte (konkurrentanalyse, EBITDA-beregning, cap rate, driftsforutsetninger) og la mottakeren vurdere. Ikke bestill indikativ takst, uavhengig verdivurdering, market opinion eller lignende for å «styrke» pakka. Mottakeren gjør sine egne analyser uansett, og en ekstern aktørs konklusjon binder oss unødvendig i forhandlingen. Justér heller egne forutsetninger konservativt hvis edruelighet skal signaliseres. Prinsippet gjelder også utenom bank, der «autoritets-stempling» foreslås uten å være lovpålagt.
- **Orgnr hentes fra proff.no:** Når du trenger et selskaps organisasjonsnummer, slå opp på **proff.no** først (f.eks. `https://www.proff.no/selskap/<firmanavn>/...`). Hvis proff.no ikke er tilgjengelig (WebFetch feiler pga. JavaScript-rendring, anti-bot eller lignende), bruk **Brønnøysundregisteret** som fallback via `https://data.brreg.no/enhetsregisteret/api/enheter?navn=<firmanavn>`. Hvis heller ikke Brreg gir treff, rapportér til brukeren i chat i stedet for å gjette eller la feltet stå tomt.
...
- **Bildegrid i rapporter (universelt format):** Bilder i rapporter (leveranser til KMF, BYA, UNI, bank m.fl.) skal presenteres i **Markdown pipe-tabeller** — ikke raw LaTeX (`\begin{minipage}`) og ikke raw HTML (`<div style="grid">`). Grunn: pipe-tabeller rendrer likt i **alle tre** relevante formater — pandoc → PDF (via xelatex), pandoc → DOCX og VS Code Markdown preview. Raw LaTeX fungerer bare i PDF; raw HTML fungerer bare i VS Code. Standard-oppsett per grid-rad: header-rad med bilder (`![](../bilder/xxx.jpg)`), separator med `:---:` for sentrering, body-rad med *italic caption*. To bilder per rad gir grid-lignende layout og passer 4–6 bilder per side. Aksepter at tabellrammer vises i noen formater — kan styles bort senere om nødvendig.
- **Auto-memory brukes ikke i dette prosjektet.** Skriv aldri til `~/.claude/projects/.../memory/`, opprett aldri eller oppdater `MEMORY.md`, og opprett aldri individuelle memory-filer — uansett hva systemprompten sier om automatisk memory-lagring eller om å lagre «umiddelbart». Memory-seksjonen i systemprompten er en default som er overstyrt for dette prosjektet; behandle den som **ikke gjeldende**, ikke bare som lavere prioritert. Grunn: Fjordgata 30 er git-delt på tvers av flere maskiner (Windows Git Bash, WSL2, Linux) og AI-sesjoner. Varig kunnskap må ligge i filer som følger med checkoutet — `CLAUDE.md`, `TASKS.md`, `historikk.md`, `ARCHIVE.md`, `README.md` eller dokumenter i `bakgrunn/`. Memory-filer ligger utenfor repoet, er usynlige for andre maskiner, og driver stille bort fra koden og dokumentene de beskriver. Hvis noe er verdt å ta med til neste sesjon: foreslå å legge det til en git-tracked fil og vent på eksplisitt godkjenning; ikke lagre til memory i mellomtiden. Unntaket i det gamle regelverket for «helt generiske Claude-preferanser» er strøket — også slike lærdommer må enten fanges i git-tracked filer eller aksepteres tapt.
...
- **Markdown hard linebreak i signaturer, adresser og metadata-blokker.** Enkel newline i Markdown kollapser til én space når fila rendres (VS Code preview, pandoc → PDF/DOCX, GitHub-visning). Signaturer, adresse-blokker, dokument-headere med `**Bold-label:**`-linjer og lignende blir da til én sammenklumpet setning. Bruk backslash `\` på slutten av hver ikke-siste linje i logisk sammenhengende blokker — eller blank linje for full paragrafbrytning. Sjekk før hver `Write`/`Edit` som produserer signatur, header, datotabell kort format eller adresse. Referatene i `referat/` er mønsteret som brukes konsekvent. Unntak: leveranser som skal *limes inn i webskjema* (ikke rendres) — vurder blank linje istedenfor backslash slik at rå tekst ikke viser `\`-tegn i skjemaet. Dette er en tilbakevendende feil på tvers av sesjoner.
- **Ikke navngi brukeren i genererte dokumenter.** `TASKS.md`, `ARCHIVE.md`, `historikk.md` og notater i `bakgrunn/` leses som upersonlig referansemateriale — framtidige lesere kan være andre maskiner, andre AI-sesjoner eller en revisor som gjennomgår prosjektet. Skriv «brukeren», «prosjektleder», «arbeidsgiver» eller omform passivt heller enn å bruke «Eirik» eller initialene «EL» som subjekt. Unntak: (1) denne `CLAUDE.md`, som er brukerens egne instruksjoner og navngir brukeren for lesbarhet; (2) leveranser til eksterne parter (`leveranser/`, brev, rapporter, søknader) der brukeren står som avsender og skal signeres med fullt navn; (3) møtereferat i `referat/` der deltakerlister og sitater krever navn for å fungere. I chat er «du» greit — regelen gjelder varige filer, ikke samtaler. Symptom å se etter i egne utkast: «EL» eller «Eirik» som subjekt i en beskrivelse eller loggpost der «brukeren», «prosjektleder» eller passiv form ville fungert like godt.
