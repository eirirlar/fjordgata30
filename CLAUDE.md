...
- **Ikke bruk `isolation: "worktree"` for read-only agenter:** Når `Agent`-tool invokeres for tasks som bare leser filer (rolle-baserte reviewer, parallell research, kodeleting, etc.) – ikke sett `isolation: "worktree"`. Agenter har uansett eget kontekstvindu, og filsystem-isolasjon gir ingen verdi når de ikke skriver. Worktrees opprettet for read-only agenter må manuelt ryddes etterpå (Claude Code-harness rydder ikke alltid pålitelig), inkludert opplåsing av stale locks og sletting av branches. Worktree-isolasjon skal kun brukes når agenten **faktisk skriver/endrer filer** og vi vil isolere disse endringene fra hovedrepoet eller andre parallelle agenter.
